 ...
 */
public class OmegatCommonmarkFilter implements IFilter {
    ...
    public static void loadPlugins() {
        Core.registerFilterClass(OmegatCommonmarkFilter.class);
    }
    ...
    void process(final BufferedReader reader) throws IOException {
        String article = IOUtils.toString(reader);
        process(article);
    }
    ...
    void process(final String testInput) {
        resetOutbuf();
        EntryHandler handler = new EntryHandler(this, testInput);
        CommonmarkVisitor visitor = new CommonmarkVisitor(handler);
        Parser processor = Parser.builder().build();
        Node node = processor.parse(handler.getArticle());
        node.accept(visitor);
        handler.finish();
    }
